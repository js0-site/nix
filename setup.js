...
const { dirname: ROOT, filename: $0 } = import.meta,
  argv = await yargs(hideBin(process.argv))
    .scriptName($0)
    .usage("用法: SSHPASS=密码 $0 <主机名> [主机名...] [--yes]")
    .option("yes", {
      alias: "y",
      type: "boolean",
      description: "跳过确认提示",
    })
    .demandCommand(1, "请至少提供一个主机名")
    .help()
    .parse(),
  { _: VPS_LI, yes: YES } = argv,
  DIR_SH = join(ROOT, "sh"),
...
const setup = async (ip, hostname) => {
  ...
  // 获取操作系统元信息
  const p = $`ssh -o ConnectTimeout=10 -o BatchMode=yes -q ${vps} bash -s`;
  ...
  const li = result.stdout.trim().split("\n"),
    os = li.pop().replace("\\l", "").replace("\\n", "").trim();

  if (os.includes("NixOS") && !os.includes("NixOS kexec-")) {
    console.log(`⚠️ ${hostname} ${ip} 已经是 NixOS (${os}) 了 ！( 无法安装 )`);
    return;
  }
...
const nixosAnywhere = async (ip, hostname, temp) => {
  write(join(temp, "flake.nix"), FLAKE.replace("/dev/null", `./nix/vps/conf/${hostname}.nix`));
...
const IP_HOSTNAME = JSON.parse(read(join(DIR_VPS, "host.json"))),
  HOSTNAME_IP = new Map(Object.entries(IP_HOSTNAME).map(([key, value]) => [value, key]));

for (const hostname of VPS_LI) {
  const ip = HOSTNAME_IP.get(hostname);
  if (!ip) {
    console.error(hostname + " 未找到IP");
    exit(1);
  }
